...
function Dashboard() {
  ...
  const handleTracker = (id) => {
    ...
    /** Handle Tracker with wallet */
    const url = `${fintrackURL}/oauth/authorize/?response_type=code&code_challenge=${fintrackCodeChallenge}&code_challenge_method=S256&client_id=${fintrackClientID}&redirect_uri=${fintrackRedirectURI}&scope=${fintrackScope}`;
          ...
          <table className="border w-full text-lg mt-6 p-4 block">
            <thead>
              <tr className="text-left text-xl">
                ...
                <th className="py-2 px-4 w-1/6">Date</th>
                  ...
                  <td className="flex items-center gap-5 py-2 px-4">
                    {
                      !transaction.uploaded ? (
                        <button type="button" onClick={() => handleTracker(transaction.id)}
                        ...
                        <p className="text-lg bg-green-600 text-white px-4 py-3">Already Uploaded</p>
                      )
                    }
                  </td>
                </tr>)
